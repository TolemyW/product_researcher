...
## 当前阶段
- **阶段：阶段 4（自动化运行与部署）**
- **状态：调度、配置加载、Docker 化与运行日志落地，开始筹备阶段 5 的性能与质量优化。**
- **最新进展概览：**
  ...
  - 摘要阶段默认消费清洗后的 normalized 数据，支持独立 normalize 命令，新增 LLM 摘要与关键词生成入口（基于 `OPENAI_API_KEY`）。
...
- [x] 要点提炼、优劣势总结、对比分析模块（基于摘要关键词的规则分类与对比速览）。
- [x] 默认中文报告模板，突出渠道/语言分布与优劣势；多语言输出暂缓。
- [x] 示例数据与快照测试，保证报告格式稳定。

### 阶段 4：自动化运行与部署
- [x] 定时/事件驱动的采集调度与任务管理（JSON 配置 + CLI `schedule`，支持一次性与轮询运行）。
- [x] 配置与敏感信息管理（env/config 文件加载与默认值）。
- [x] Docker/Compose 一键部署（挂载 data/logs/config）。
- [x] 监控与告警基础（运行日志记录，便于接入聚合/通知）。
